### 内部排序
  ...
  |类别|排序方法|时间复杂度|辅助空间|稳定性|
  |:-----|:-----:|:-----:|:-----:|-----:|
  |插入排序|直接插入|O<sub>(n<sup>2</sup>)</sub>|O<sub>(1)</sub>|稳定|
  |插入排序|希尔排序|O<sub>(n<sup>1.3-2</sup>)</sub>|O<sub>(1)</sub>|不稳定|  
  |选择排序|简单选择|O<sub>(n<sup>2</sup>)</sub>|O<sub>(1)</sub>|不稳定|
  |选择排序|堆排序||||  
  |交换排序|冒泡排序|O<sub>(n<sup>2</sup>)</sub>|O<sub>(1)</sub>|稳定|
  |交换排序|快速排序||||
  |其他|归并排序||||
  |其他|基数排序||||
### 简单排序
