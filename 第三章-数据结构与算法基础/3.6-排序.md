...
### 简单排序
  ...
  + 冒泡排序★★★
   ...
   + 快速排序★★★
     + 分治法
     + 思路:取数组中任意一个基数，将其他数值跟基数对比后，比基数大的放基数后面，比基数小的放基数前面
     + 设立首尾两个位置指示器i和j分别指向数组的头和尾
     + 取一个标准基值(一般是取数组的第一个的**值**Data[0])qivot=Data[0]
     + j指针指向的Data跟qivot比较大小，如果Data[j]>=qivot，Data[j]所在位置在qivot之后，则不需要改变Data[j]值得位置，j--往前推进一位继续判断，直到Data[j]<qivot,交换i和j位置的元素
